...
import { restaurantPath } from './legalNavigation.ts';
...
export function LegalDocumentPage({ type }: { type: LegalDocumentType }) {
  ...
  const returnTo = restaurantPath(searchParams.get('restaurant'));

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:py-12">
        <Link to={returnTo} className="inline-flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-bold text-teal-800 hover:bg-teal-50 focus:outline-none focus:ring-4 focus:ring-teal-100"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> Voltar para o restaurante</Link>
